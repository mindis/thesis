...
def recommend(df,userID, user_item, user_vecs, item_vecs, num_items=10):
...
if __name__ == "__main__":
    ...
    PATH = '/home/nick/Desktop/thesis/datasets/pharmacy-data/user_product_ratings.csv'
    ...
    user_id = 50
    data[userkey] = data.groupby(userkey).ngroup()
    data[itemkey] = data.groupby(itemkey).ngroup()
    data.drop(columns=['Unnamed: 0','user','item'],inplace=True)
    print(data)
    recommendations = recommend(data,user_id, user_item, user_vecs, item_vecs)
    recommendations['item'] = recommendations.item.astype(str)
    """Merge indexed product with real product for user = userID"""
    recommendations = recommendations.merge(item_lookup, on='item')
